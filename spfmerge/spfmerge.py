...
def _is_fqdn(string) -> bool:
    ...
    Returns:
        bool: True if string matches FQDN syntax
    """
    return FQDN(string).is_valid


def _is_ipv4(string) -> bool:
    """Check if a string is a valid IPv4 network or address

    Args:
        string (str): String to be validated

    Returns:
        bool: True if string matches a valid IPv4 network or address syntax
    """
    try:
        return isinstance(ip_network(string)) == IPv4Network or isinstance(
            ip_address(string) == IPv4Address
        )
    except ValueError:
        return False


def _is_ipv6(string) -> bool:
    """Check if a string is a valid IPv6 network or address

    Args:
        string (str): String to be validated

    Returns:
        bool: True if string matches a valid IPv6 network or address syntax
    """
    try:
        return isinstance(ip_network(string)) == IPv6Network or isinstance(
            ip_address(string) == IPv6Address
        )
    except ValueError:
        return False
...
class SPFRecord:
    ...
    SPF_SYNTAX_REGEX_STRING = (
        r"^v=spf1( +([-+?~]?(all|include:(%\{[CDHILOPR-Tcdhilopr-t]([1-9][0-9]?|10[0-9]|11[0-9]"
    ...
    def generate_txt(self):
        ...
        for mechanism_name, mechanism_entries in self.mechanisms.items():
            for mechanism_entry in mechanism_entries:
                ...
                if (
                    ...
                    and isinstance(mechanism_entry["value"]) == str
                ):
